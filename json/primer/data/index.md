...
1. It is [JSON data](../../data). It follows the rules of the JSON
   specification.
2. It is NIEM data. Its meaning is given by its content model, which
   is based on the NIEM conceptual model.
...
is a collection of assertions about some object in the world. For each
pair in the JSON object, the key string represents an object property,
and the corresponding value is the value of that property. The order
of the pairs is not significant. The values can be:
...
```
http://releease.niem.gov/niem/niem-core/4.0/#PersonSurName
```

Which is the global identifier for the data element defined as "A last
name or family name of a person". The data elements
...
### Connecting NIEM JSON data to its JSON-LD context

NIEM JSON data is always associated with its JSON-LD context.
One approach is to embed the context object within the NIEM JSON
data.  For *Example 2*, the result would look like:
...
As an alternative, NIEM JSON data may include the URI of its context
